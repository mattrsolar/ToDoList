...
export const buttonVariants = cva("flex item-center justify-center cursor-pointer transition rounded-lg group gap-2",{
    variants:{
        variant:{
...
function Button({
    ...
    return (
        <button className={buttonVariants({variant, size, disabled, className})} {...props}>
            {IconComponent && <Icon className={buttonIconVariants({variant, size})} svg={IconComponent}/>}
